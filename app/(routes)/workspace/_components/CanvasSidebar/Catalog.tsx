...
const ComponentCatalog: React.FC<ComponentCatalogProps> = ({ isOpen, onClose, onSelectComponent }) => {
  ...
  // Fetch permissions from the Kind client
  const { getPermissions } = useKindeBrowserClient();

  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        const permissionResponse = await getPermissions();  // assuming getPermissions is an async function
        console.log("User permissions:", permissionResponse);
        // setPermissions(permissionResponse); // Set the fetched permissions
      } catch (error) {
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            ...
            {/* Conditionally render Add button based on permissions */}
            {hasPermission(requiredPermissions.add) && (
              <button
                ...
                >
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex gap-2">
                      {/* Conditionally render Edit and Delete buttons based on permissions */}
                      {hasPermission(requiredPermissions.edit) && (
                        <button onClick={handleEdit} className="text-yellow-600 hover:text-yellow-700">
                          <Edit className="w-5 h-5" />
                        </button>
                      )}
                      {hasPermission(requiredPermissions.delete) && (
                  ...
                  {expandedSpecs[component.id] && (
                    <div className="mt-2 space-y-2">
                      ...
                      {/* Conditionally render Add to Sidebar button */}
                      <button
